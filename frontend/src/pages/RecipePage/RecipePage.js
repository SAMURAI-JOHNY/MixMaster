...
const RecipePage = () => {
  ...
  const calculateIngredients = () => {
    ...
    // Ингредиенты приходят как объекты RecipeIngredient с ingredient_id и ingredient объектом
    return selectedRecipe.ingredients.map(ingredient => {
      const volume = (ingredient.volume_ml || ingredient.volume || 0) * portions;
      const name = ingredient.ingredient?.name || ingredient.name || `Ингредиент ${ingredient.ingredient_id}`;
      return {
        name,
        volume,
        unit: 'мл'
    ...
    try {
      ...
      if (!checkResult.available) {
        const missingList = checkResult.missing_ingredients
          .map(ing => `${ing.ingredient_name}: требуется ${ing.required_volume}мл, доступно ${ing.available_volume}мл`)
          .join('\n');
        alert(`Недостаточно ингредиентов для ${portions} ${portions === 1 ? 'батча' : 'батчей'}:\n${missingList}`);
      } else {
  ...
  const handlePrepareCocktail = async () => {
    ...
    if (!window.confirm(`Приготовить коктейль "${cocktail.name}" в количестве ${portions} ${portions === 1 ? 'батча' : 'батчей'}? Ингредиенты будут списаны.`)) {
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        // Проверяем, был ли коктейль уже приготовлен ранее
        const preparedCheck = await preparedCocktailsAPI.checkIfPrepared(id);
        setIsPrepared(preparedCheck.is_prepared);
        alert(result.message || 'Коктейль успешно приготовлен! Ингредиенты списаны.');
        // Перезагружаем данные для обновления остатков
      ...
      if (err.response?.status === 400) {
        const missingList = err.response?.data?.missing_ingredients
          ?.map(ing => `${ing.ingredient_name}: требуется ${ing.required_volume}мл, доступно ${ing.available_volume}мл`)
          .join('\n') || '';
  ...
  const handleDeleteRecipe = async () => {
    ...
    if (!window.confirm(`Вы уверены, что хотите удалить рецепт "${selectedRecipe.type || 'Рецепт'}"?`)) {
      return;
    }
  ...
  const handleDeleteCocktail = async () => {
    if (!window.confirm(`Вы уверены, что хотите удалить коктейль "${cocktail.name}"? Все связанные рецепты также будут удалены. Это действие нельзя отменить.`)) {
      return;
    }
    try {
        ...
        <div className="recipe-header-section">
          <div className="recipe-text-content">
            ...
            <p className="cocktail-description">
              {cocktail.description || 'Описание отсутствует'}
            </p>
          </div>
          <div className="recipe-image-content">
            <img src={cocktail.image_url ? uploadAPI.getImageUrl(cocktail.image_url) : CocktailImg} alt={cocktail.name} className="cocktail-image" />
          </div>
        </div>
                  ...
                  <table className="ingredients-table">
                    <tbody>
                      {ingredients.map((ingredient, index) => (
                        <tr key={index}>
                          <td className="ingredient-name">{ingredient.name || `Ингредиент ${index + 1}`}</td>
          ...
          <section>
            {isPrepared && (
              <div className="prepared-badge">
                ✓ Этот коктейль уже был приготовлен
              </div>
            )}
            <div className="portions-control">
              ...
              <div className="action-buttons-group">
                ...
                <button 
                  ...
                  className="recipe-action-button stock-button"
                >
                  Проверить остатки
                </button>
              </div>
