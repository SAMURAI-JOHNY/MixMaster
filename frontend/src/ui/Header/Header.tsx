...
export const Header = () => {
  ...
  const [userAvatarStorageUrl, setUserAvatarStorageUrl] = useState(null);
  const [userRole, setUserRole] = useState(null);

  const handleAvatarUpdated = useCallback((url) => {
    setUserAvatarStorageUrl(url);
  }, []);
      ...
      if (savedLogin === 'true' && savedUsername) {
        ...
        if (tokenCheck.valid) {
          ...
          try {
            ...
            setUserAvatarStorageUrl(profile.avatar_url ?? null);
            if (profile.role) {
    ...
    <header className="header">
      <div className="header-content">
        <div className="header-left">
          ...
          <div className={`dropdown-menu ${isMenuOpen ? 'open' : ''}`}>
            <nav className="menu-nav">
              ...
              <button className="menu-item" onClick={() => handleNavigation('/ingredients')}>
                Ингредиенты
              </button>
              {isLoggedIn && userRole === 'бармен' && (
                <button className="menu-item" onClick={() => handleNavigation('/create-recipe')}>
                  Создать рецепт
                </button>
              )}
              {isLoggedIn && (
                <button className="menu-item" onClick={() => handleNavigation('/my-prepared')}>
                  Приготовленные
                </button>
              )}
            </nav>
        ...
        <div className="header-right">
          <button className="avatar-button" onClick={toggleAuthModal} title={isLoggedIn ? `Профиль (${username})` : 'Войти в систему'}>
            <ResolvedImage
              key={userAvatarStorageUrl ?? 'no-avatar'}
              storageUrl={userAvatarStorageUrl}
              fallbackSrc={DefaultAvatarIcon}
              alt="Аватар"
              className={userAvatarStorageUrl ? 'user-avatar-img' : 'default-avatar'}
            />
          </button>
        </div>
      </div>
